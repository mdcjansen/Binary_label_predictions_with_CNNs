...
class StratifiedKFoldDatasetSplitter:
    ...
    def _save_fold_data(self, train_study_ids, val_study_ids, fold):
        base_save_dir = os.path.join(os.path.dirname(self.image_dir), save_dir_name)
        train_save_dir = os.path.join(base_save_dir, f'Fold-{fold}', 'Train')
        val_save_dir = os.path.join(base_save_dir, f'Fold-{fold}', 'Val')
    ...
    def _log_fold_statistics(self, train_study_ids, val_study_ids, test_study_ids, labels, fold):
        ...
        test_img_count, test_study_count, test_class_dist = self._get_statistics_test(test_study_ids, labels, fold, 'Test')
        ...
        logging.info(f"Test: Image patches={test_img_count}, Unique Study ID={test_study_count}, Class Distribution={test_class_dist}")
        logging.info("\n")
        ...
        stats_df = pd.DataFrame({
            ...
            'Class_Distribution': [str(train_class_dist.tolist()), str(val_class_dist.tolist()), str(test_class_dist.tolist())]
        })
    ...
    def _get_statistics_test(self, study_ids, labels, fold, subset):
        img_count = sum(len(files) for _, _, files in os.walk(os.path.join(os.path.dirname(self.image_dir), save_dir_name, "Test", subset)))
    ...
    def _get_statistics(self, study_ids, labels, fold, subset):
        img_count = sum(len(files) for _, _, files in os.walk(os.path.join(os.path.dirname(self.image_dir), save_dir_name, f'Fold-{fold}', subset)))
        study_count = len(set(study_ids))
        class_dist = np.bincount([labels[sid] for sid in study_ids])
    ...
    def verify_stratification(self):
        ...
        for fold in range(1, self.n_splits + 1):
            fold_labels = []
            for _, _, filenames in os.walk(os.path.join(self.image_dir, f'Fold-{fold}', 'Train')):
                for filename in filenames:
                    if filename.endswith('.jpg'):
                        img_name = os.path.splitext(filename)[0]
                        sid = int(img_name.split('_')[1])
...
stratified_kfold_splitter = StratifiedKFoldDatasetSplitter(image_dir, excel_file, n_splits=4)