...
def create_model(batch_norm, dropout_rate, fine_tune_last_n=10):
    ...
    # Unfreeze last N blocks of the last stage
    ...
    ## Check with ConvNextTiny before 231109
    # =============================================================================
    #     # Determine the number of features before the custom head
    #     dummy_input = torch.randn(1, 3, 299, 299, device=device)
    #     with torch.no_grad():
    #         model.eval()
    #         features = model.forward_features(dummy_input)
    #         features_flattened = features.reshape(features.size(0), -1)  # Updated line
    #         input_features = features_flattened.size(1)
    #
    #     # Initialize custom head with the correct input size
    #     model.head = CustomHead(input_features, 1, batch_norm, dropout_rate).to(device)  # Ensure the head is also on the correct device
    # =============================================================================
